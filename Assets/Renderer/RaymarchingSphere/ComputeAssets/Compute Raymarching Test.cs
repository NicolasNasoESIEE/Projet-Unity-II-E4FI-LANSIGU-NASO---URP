...
public class ComputeRaymarchingSphere : URPComputeAsset
{
    ...
    int kernelHandle)
    {
        ...
        commandBuffer.SetComputeMatrixParam(shader,
        "CameraToWorld", camera.cameraToWorldMatrix);
        commandBuffer.SetComputeMatrixParam(shader,
        "CameraInverseProjection",
        camera.projectionMatrix.inverse);
        commandBuffer.SetComputeVectorParam(shader,
        "SurfaceColor",
        new Vector3(surfaceColor.r, surfaceColor.g,
        surfaceColor.b));
        Vector3 lightDirection = Quaternion.Euler(lightRotation) * Vector3.forward;
        commandBuffer.SetComputeVectorParam(shader,
        "DirectionalLight", new Vector4(lightDirection.x,
        lightDirection.y, lightDirection.z, lightIntensity));
    }
}
